...
import time
def formatJson(obj, level=0):
    indent = "    " * level
    if isinstance(obj, dict):
        items = []
        for k, v in obj.items():
            items.append(f'{indent}    "{k}": {formatJson(v, level + 1)}')
        return "{\n" + ",\n".join(items) + f"\n{indent}}}"

    elif isinstance(obj, list):
        items = [formatJson(item, level + 1) for item in obj]
...
for i in range(100):
    with open('Schedule.bin', 'rb') as fileIn:
        with open('Schedule.json', 'w') as fileOut:
            a = formatJson(binaryToJson(fileIn.read()), 0)
            fileOut.write(a)
